<?php 
    ...
    echo "Fecha de pedido\tForma de Pago\tAccion del socio\tNombre del socio\tSolicitud de llamada\tOrigen\tDestino\tColonia\tKm\tTotal del viaje\tComentario\tConductor\tUnidad\tViaje\n";

    //Consulta SQL
    $querySVe   = "SELECT fecha, formadepago, accion, nombredelsocio, solicituddellamada, origen, destino, colonia, km, sumatotaldeviaje, comentario, conductor, unidad, viaje FROM registro";
    ...
    if ($resultadoSVe->num_rows > 0) {
        while ($filaSVe = $resultadoSVe->fetch_assoc()) {
            echo $filaSVe['fecha'] . "\t" . $filaSVe['formadepago'] . "\t" . $filaSVe['accion'] . "\t" . $filaSVe['nombredelsocio'] . "\t" . $filaSVe['solicituddellamada'] . "\t" . $filaSVe['origen'] . "\t" . $filaSVe['destino'] . "\t" . 
                $filaSVe['colonia'] . "\t" . $filaSVe['km'] . "\t" . $filaSVe['sumatotaldeviaje'] . "\t" . $filaSVe['comentario'] . "\t" . $filaSVe['conductor'] . "\t" . $filaSVe['unidad'] . "\t" . $filaSVe['viaje'] . "\t" . "\n";
        }
    } else {
